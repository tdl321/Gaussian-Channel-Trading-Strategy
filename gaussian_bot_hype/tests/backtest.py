...
class GaussianChannelStrategy(bt.Strategy):
    ...
    - 6 poles, 144 period, 1.414 multiplier (original)
    ...
    params = (
        ('poles', 6),                    # Number of poles for Gaussian filter (original)
        ('period', 144),                 # Sampling period for Gaussian filter (original)
        ('multiplier', 1.414),           # Band width multiplier (original)
        ('leverage', 5),                 # Leverage multiplier (from config)
        ('position_size_pct', 1.0),      # Position size as percentage of portfolio (100%)
        ('atr_period', 14),              # ATR calculation period (original)
...
def get_asset_strategy_params(symbol):
    ...
    """
    if symbol == 'BTC':
        # BTC parameters (original, proven most effective)
        return {
